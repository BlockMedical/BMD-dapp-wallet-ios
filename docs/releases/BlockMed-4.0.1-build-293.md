...
App Review submission is complete. TestFlight launch verification on the affected iPhone remains pending owner confirmation.
...
- TestFlight launch result: pending owner verification on the affected iPhone
...
Pending TestFlight verification:

1. On the affected iPhone, refresh TestFlight, install build 293, and launch BlockMed without deleting the existing app first. Testing as an in-place update is necessary to exercise the preserved Realm database.
2. Confirm launch, wallet list display, background/foreground behavior, and a second cold launch.
3. Record the observed result in this release record.
